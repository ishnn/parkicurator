...
def createModel(train_data=None):
    if os.path.exists('wave.h5') and train_data is None:
        try:
            print(__name__)
            model = keras.models.load_model('wave.h5')
            print("returned")
            return model
...
def getData():
    rawdata = []
    data = []
    dir = "D://FREELANCE PROJECTS//PARKINSON DISEASE DETECTION//DATASET//wave"
...
checkpoint = keras.callbacks.ModelCheckpoint(filepath='wave.h5', save_best_only=True, monitor='val_loss',
                                             mode='min')
